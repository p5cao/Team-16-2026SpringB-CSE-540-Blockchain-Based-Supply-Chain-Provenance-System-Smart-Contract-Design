...
router.get('/', async (req, res) => {
  ...
  const rows = await resolveOwnerRoles(db.prepare(sql).all(...params))
...
router.get('/:prodId/history/ownership', (req, res) => {
  ...
  const rows = db.prepare('SELECT * FROM ownership_history WHERE prod_id = ? ORDER BY block_number ASC').all(id)
  res.json({ prod_id: id, count: rows.length, history: rows })
})
...
router.get('/:prodId/history/status', (req, res) => {
  ...
  const rows = db.prepare('SELECT * FROM status_history WHERE prod_id = ? ORDER BY block_number ASC').all(id)
  res.json({ prod_id: id, count: rows.length, history: rows })
})
